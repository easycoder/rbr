...
# Usage: ./tests/run-setup-tests.sh [playwright args...]
#   e.g. ./tests/run-setup-tests.sh --headed
#        ./tests/run-setup-tests.sh --grep "banner"
...
# 4. Run Playwright tests (setup specs only)
npx playwright test tests/setup-*.spec.js "$@"
TEST_EXIT=$?
