...
test.describe('Living Room', () => {

    test('add and configure Living Room', async ({ page }) => {
        page.on('console', msg => {
        ...
        // Wait for Kitchen to appear (from previous tests)
        await expect(page.locator('#room-name-0')).toHaveText('Kitchen', { timeout: 20000 });
        ...
        // New room should appear as "Unnamed" at index 1
        await expect(page.locator('#room-name-1')).toHaveText('Unnamed', { timeout: 15000 });

        // --- Rename to "Living Room" ---
        await page.locator('#room-tools-1').click();
        ...
        await dialogInput.fill('Living Room');
        await page.locator('#dialog-button1').click();

        // Wait for the UI to return to the main screen
        ...
        await sensorInput.fill('Living-Room-thermometer');

        await page.locator('#relayType').selectOption('Zigbee');
        await page.locator('#dialog-relayList').fill('Living-Room-radiator');
        await page.locator('#dialog-linked').check();
        await page.locator('#dialog-button1').click();

        // Verify both rooms are showing
        ...
        await expect(page.locator('[id^="room-name-"]')).toHaveCount(2);
    });
});
