...
void onDefault() {
  ...
  sprintf(info, "RBR R1 v%d '%s' RSSI=%d %s/%s ",
    CURRENT_VERSION, my_id, WiFi.RSSI(), host_ssid, host_ipaddr);
  showRelayState(info);
}
...
void connectToHost() {
  Serial.printf("'%s' %s %s %s %s %s\n", my_id, host_ssid, host_password, host_ipaddr, my_ssid, my_password);
  ...
  // Set up the poll ticker
  pollTicker.attach(POLL_INTERVAL, doPoll);
}
...
void setup() {
  ...
  if (config_p != "") {
    ...
    if (error) {
      ...
      getConfigValue(config, "my_id", my_id);
      ...
      if (my_id[0] == '\0' || my_password[0] == '\0' || host_ssid[0] == '\0' || host_password[0] == '\0'
...
void loop() {
  ...
  if (pollFlag) {
    pollFlag = false;
    char url[80];
    sprintf(url, "http://%s/psu1/poll/%s", host_server, my_id);
  ...
  if (checkForUpdate) {
    ...
    char url[80];
    sprintf(url, "http://%s/psu1/version", host_server);
    char* response = httpGET(url);
    int newVersion = atoi(response);
    free(response);
