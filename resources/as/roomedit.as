!	Room Editor
!	Manages a menu of actions all relating to room characteristics,
!	specifically to set the name of the room, identify the thermometer and relay(s)
!	used in the room, and call the period editor.
    ...
    input P100Email
    input P100Password
    input LinkedCheckbox
    input ProtectCheckbox
    textarea RelayList
    select RelayTypes
    variable Profiles
    variable Profile
...
NameButtonCB:
    ...
    attach KeyboardButton to `dialog-keyboard`
...
DevicesButtonCB:
    ...
    attach DialogTitle to `dialog-title`
    attach DialogText to `dialog-text`
    attach DialogText2 to `dialog-relayListTitle`
    attach RelayList to `dialog-relayList`
    attach DialogButton1 to `dialog-button1`
    attach DialogButton2 to `dialog-button2`
    ...
    attach LinkedCheckbox to `dialog-linked`
    attach ProtectCheckbox to `dialog-winter-protect`
    attach LinkedDiv to `linked-div`
    if property `relayType` of Room is empty set style `display` of LinkedDiv to `none`
    if property `linked` of Room is not `no` set attribute `checked` of LinkedCheckbox to `checked`
    if property `protect` of Room is `yes` set attribute `checked` of ProtectCheckbox to `checked`
    set the content of ProtectTemp to  property `ptemp` of Room
    
    put `[]` into List
    json add `none` to List
    json add `RBR-Now` to List
    json add `Shelly One` to List
    json add `Zigbee` to List
    ...
    begin
        ...
        begin
            ...
            set style `display` of RelayList to `block`
        end
        if attribute `value` of RelayTypes is `none`
        	set style `display` of LinkedDiv to `none`
        else
        	set style `display` of LinkedDiv to `block`
    end
    
    set the content of DialogTitle to `Sensor and relays for '` cat property `name` of Room cat `'`
    set the content of DialogText to `The temperature sensor (name or address)`
    set the content of DialogText2 to `The relay(s) (name, address etc), 1 per line`
    ...
    set the content of RelayList to Item
    
    on click DialogButton1
    begin
    	put `{}` into Items
        set property `sensor` of Items to the content of DialogInput
        if attribute `checked` of LinkedCheckbox
        begin
        	if the content of ProtectTemp is empty set the content of ProtectTemp to `0.0`
        	set property `linked` of Items to `yes`
        end
        else set property `linked` of Items to `no`
        if attribute `checked` of ProtectCheckbox
        begin
        	if the content of ProtectTemp is empty set the content of ProtectTemp to `0.0`
        ...
        put `[]` into Relays
        split the content of RelayList on newline giving Item
        put 0 into R
        while R is less than the elements of Item
        begin
            index Item to R
            if Item is not empty append Item to Relays
            add 1 to R
        end
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!	Edit the times. The legacy schedule editor was removed when the
!	codebase moved to periods-only; this button now reports that
!	editing must happen in the new UI.
TimesButtonCB:
    alert `Schedule editing has moved to the new UI.`
    exit
